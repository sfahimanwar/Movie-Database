...
router.get('/movies/:movie',function(req,res){
    ...
    if(objCopy != null){
        objCopy.reviews = model.getMoviesReviews(model.getMovieByName(movieParam).movieID);
    }
    res.json(objCopy);
...
router.get('/users/:user',function(req,res){
   ...
   if(objCopy != null){
       objCopy.reviews = model.getUsersReviews(model.getUserByName(userParam).userID);
   }
   let accountType;
   let newUserObj;
...
router.get('/movies',function(req,res){
    ...
    if(!/\?.+/.test(req.url)){
        ...
        found = (model.getMoviesByQueryParameters(req.query.title,req.query.genre,req.query.year,req.query.minrating)).map(elem=>elem.title);
    }
    res.json(found);
});
...
router.post('/movies',function(req,res){
    if(model.addMovieAPI(req.body.title, req.body.year, req.body.runtime, req.body.plot, req.body.genre1, req.body.genre2, req.body.actor1,
        req.body.actor2, req.body.actor3, req.body.director, req.body.writer)){
        res.json(model.getMovieByName(req.body.title));
    }else{
        res.status(400).send('Invalid JSON Data or movie already exists. Follow documentation.');
    }
});
