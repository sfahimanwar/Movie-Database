...
function signUp(username, password, genre1, genre2, genre3) {
  ...
  if (
    ...
    let newString1 =
      genre1.toLowerCase().charAt(0).toUpperCase() +
      genre1.toLowerCase().slice(1);
    let newString2 =
      genre2.toLowerCase().charAt(0).toUpperCase() +
      genre2.toLowerCase().slice(1);
    let newString3 =
      genre3.toLowerCase().charAt(0).toUpperCase() +
      genre3.toLowerCase().slice(1);
...
function followUser(userID, toFollowUserID) {
  ...
  if (
    users.some((elem) => elem.userID === userID) &&
    users.some((elem) => elem.userID === toFollowUserID) &&
    userID != toFollowUserID &&
    !users[userID].followingUsers.includes(toFollowUserID)
  ) {
...
function unfollowUser(userID, toUnfollowUserID) {
  ...
  if (users[userID].followingUsers.includes(toUnfollowUserID)) {
    users[userID].followingUsers = users[userID].followingUsers.filter(
      (elem) => elem != toUnfollowUserID
    );
    users[toUnfollowUserID].followers = users[
      toUnfollowUserID
    ].followers.filter((elem) => elem != userID);
    return true;
  } else {
    error();
    return false;
...
function searchUsers(searchParameter) {
  if (searchParameter != undefined) {
    let found = users.filter((elem) =>
      elem.username.toLowerCase().includes(searchParameter.toLowerCase())
    );
...
function calculateAvgRating(movieID, rating) {
  ...
  movies[movieID].averageRating =
    movies[movieID].totalRating / movies[movieID].noOfRatings;
}
...
function addFullReview(userID, movieID, rating, summary, fullReview) {
  ...
  if (
    users.some((elem) => elem.userID === userID) &&
    movies.some((elem) => elem.movieID === movieID) &&
    rating != undefined &&
    rating >= 0 &&
    rating <= 10 &&
    summary != undefined &&
...
function recommendMovies(userID) {
  ...
  if (users.some((elem) => elem.userID === userID)) {
    recommended1 = movies.filter((elem) =>
      elem.genre.includes(users[userID].favouriteGenres[0])
    );
    recommended2 = movies.filter((elem) =>
      elem.genre.includes(users[userID].favouriteGenres[1])
    );
    recommended3 = movies.filter((elem) =>
      elem.genre.includes(users[userID].favouriteGenres[2])
    );
...
function searchMovie(searchParameter) {
  if (searchParameter != undefined) {
    let found = movies.filter((elem) =>
      elem.title.toLowerCase().includes(searchParameter.toLowerCase())
    );
...
function getMovieByName(movieName) {
  if (movieName != undefined) {
    let found = movies.find(
      (elem) => elem.title.toLowerCase() === movieName.toLowerCase()
    );
    return found;
  } else {
    error();
...
function filterMovies(movieArray, criteria, order) {
  if (movieArray != undefined && movieArray.length != 0) {
    if (criteria === "Runtime") {
      ...
      if (order === "Ascending") {
        movieArray.sort(function (a, b) {
          return parseInt(a.yearReleased) - parseInt(b.yearReleased);
        });
      } else if (order === "Descending") {
        movieArray.sort(function (a, b) {
          return parseInt(b.yearReleased) - parseInt(a.yearReleased);
        });
      }
...
function addMovieHelper(
  ...
  if (
    person.toLowerCase() != otherPerson1.toLowerCase() &&
    !existingObj.collaborators.some(
      (elem) => elem.toLowerCase() === otherPerson1.toLowerCase()
    )
  ) {
    existingObj.collaborators.push(otherPerson1);
  }
  if (
    person.toLowerCase() != otherPerson2.toLowerCase() &&
    !existingObj.collaborators.some(
      (elem) => elem.toLowerCase() === otherPerson2.toLowerCase()
    )
  ) {
    existingObj.collaborators.push(otherPerson2);
  }
  if (
    person.toLowerCase() != otherPerson3.toLowerCase() &&
    !existingObj.collaborators.some(
      (elem) => elem.toLowerCase() === otherPerson3.toLowerCase()
    )
  ) {
    existingObj.collaborators.push(otherPerson3);
  }
  if (
    person.toLowerCase() != otherPerson4.toLowerCase() &&
    !existingObj.collaborators.some(
      (elem) => elem.toLowerCase() === otherPerson4.toLowerCase()
    )
  ) {
...
function addMovie(
  ...
  if (
    users.some((elem) => elem.userID === userID) &&
    users[userID].isContributing === true &&
    title != undefined &&
    ...
    for (let i = 0; i < movies.length; i++) {
      ...
      if (
        movies[i].genre.includes(newString1) ||
        movies[i].genre.includes(newString2)
      ) {
...
function addMovieAPIHelper(
  ...
  if (
    !people.some((elem) => elem.name.toLowerCase() === person.toLowerCase())
  ) {
...
function searchByGenre(genre) {
  if (genre != undefined && genre.length > 0) {
    ...
    if (genre != "Sci-Fi") {
      let newString =
        genre.toLowerCase().charAt(0).toUpperCase() +
        genre.toLowerCase().slice(1);
...
function getMoviesByQueryParameters(title, genre, year, minrating) {
  ...
  if (minrating != undefined) {
    results = results.filter(
      (elem) => elem.averageRating >= parseFloat(minrating)
    );
  }
  return results;
}
...
function addActor(userID, movieID, actor) {
  ...
  if (
    users[userID].isContributing &&
    people.some((elem) => elem.name.toLowerCase() === actor.toLowerCase()) &&
    !movies[movieID].actors.some(
      (elem) => elem.toLowerCase() === actor.toLowerCase()
    )
  ) {
...
function addWriter(userID, movieID, writer) {
  ...
  if (
    users[userID].isContributing &&
    people.some((elem) => elem.name.toLowerCase() === writer.toLowerCase()) &&
    !movies[movieID].writers.some(
      (elem) => elem.toLowerCase() === writer.toLowerCase()
    )
  ) {
...
function unfollowPerson(userID, toUnfollowPersonID) {
  if (
    users.some((elem) => elem.userID === userID) &&
    users[userID].followingPeople.includes(toUnfollowPersonID)
  ) {
    users[userID].followingPeople = users[userID].followingPeople.filter(
...
function searchPeople(searchParameter) {
  if (searchParameter != undefined) {
    let found = people.filter((elem) =>
      elem.name.toLowerCase().includes(searchParameter.toLowerCase())
    );
    return found;
  } else {
    error();
...
function initializationScript() {
  ...
  addFullReview(
    ...
    7.5,
    "Magical Moments",
    "My childhood story. Buzz light year and woody is epic"
  );
}
